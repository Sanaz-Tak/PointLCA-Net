#!/usr/bin/env python3
"""
DVS128 Training Script
Trains PointNet on pre-processed DVS128 H5 files.
"""
...
def parse_args():
    parser = argparse.ArgumentParser('DVS128 training with processed data')
    parser.add_argument('--use_cpu', action='store_true', default=False, help='use cpu mode')
    parser.add_argument('--gpu', type=str, default='0', help='specify gpu device')
    parser.add_argument('--batch_size', type=int, default=24, help='batch size in training')
    parser.add_argument('--model', default='pointnet_cls', help='model name [default: pointnet_cls]')
    parser.add_argument('--num_category', default=11, type=int, help='DVS128 has 11 classes')
    ...
    parser.add_argument('--data_dir', default='./processed_data/dvs128', help='Directory with processed H5 files')
...
class ProcessedDVS128Dataset(Dataset):
    """DVS128 dataset loading from processed H5 files"""
    
    def __init__(self, data_dir, split='train', transform=None):
        super(ProcessedDVS128Dataset, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        points = torch.from_numpy(self.data[idx]).float()  # Shape: (3, 1024)
        label = int(self.labels[idx])
        
        # PointNet expects (3, 1024) format, which we already have
