...
"""
SHD Training Script
Trains PointNet on pre-processed SHD H5 files.
"""
...
def parse_args():
    parser = argparse.ArgumentParser('SHD training with processed data')
    parser.add_argument('--use_cpu', action='store_true', default=False, help='use cpu mode')
    parser.add_argument('--gpu', type=str, default='0', help='specify gpu device')
    parser.add_argument('--batch_size', type=int, default=24, help='batch size in training')
    parser.add_argument('--model', default='pointnet_cls', help='model name [default: pointnet_cls]')
    parser.add_argument('--num_category', default=20, type=int, help='SHD has 20 classes')
    ...
    parser.add_argument('--data_dir', default='./processed_data/shd', help='Directory with processed H5 files')
...
def test(model, loader, num_class=20):
    mean_correct = []
...
class ProcessedSHDDataset(Dataset):
    """SHD dataset loading from processed H5 files"""
    
    def __init__(self, data_dir, split='train', transform=None):
        super(ProcessedSHDDataset, self).__init__()
        self.data_dir = data_dir
        self.split = split
        self.transform = transform
        
        # Load processed H5 file
        h5_file = os.path.join(data_dir, f'shd_{split}.h5')
        if not os.path.exists(h5_file):
            raise FileNotFoundError(f"Processed SHD {split} file not found: {h5_file}")
        
        with h5py.File(h5_file, 'r') as f:
            self.data = f['data'][:]      # Shape: (N, 3, 1024)
            self.labels = f['label'][:]    # Shape: (N,)
        
        # Ensure data is in the correct format for PointNet: (N, 3, 1024)
        # Current: (N, 3, 1024) -> Target: (N, 3, 1024) - already correct for PointNet
        # No transpose needed - PointNet expects (batch_size, channels, num_points)
...
def main(args):
    ...
    # Create datasets
    train_dataset = ProcessedSHDDataset(args.data_dir, 'train')
    test_dataset = ProcessedSHDDataset(args.data_dir, 'test')
